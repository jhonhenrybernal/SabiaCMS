...
const form = useForm({
  ...
  language: props.message.language,
  source: props.message.source ?? '',
})

const submit = () => form.put(`/admin/messages/${props.message.id}`)
</script>
    ...
    <div class="mt-6 rounded-lg border bg-white p-6">
      <form class="space-y-4" @submit.prevent="submit">
        <div>
          ...
          <textarea v-model="form.content" rows="6" class="mt-1 w-full rounded-md border px-3 py-2 text-sm" />
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div>
            <label class="text-sm font-medium">Fuente</label>
